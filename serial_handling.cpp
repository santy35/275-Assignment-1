...
"UPDATED WITH GET_PATH_LEN. UNTESTED"
...
int16_t srv_get_pathlen(LonLat32 start, LonLat32 end) {
    ...
    while(true){
      if(millis()-time_elapsed > path_len_timeout) return -1;
      ...
      if( str=='\n' || str=='\r'){
        if(buffer[0]=='N') return int(buffer[2]); //Assuming proper formatting
