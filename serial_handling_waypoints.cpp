...
int16_t srv_get_waypoints(LonLat32* waypoints, 
    ...
    //This is just a statement to confirm recieved points
    for (int16_t i=0; i < path_len; ++i) {     //************ PLUS ONE?? SHOULD CHANGE
        lon = i;
        lat = i;
        ...
        if ( i < max_path_len ) {
            ...
            while(true){
                //Timout is reached
                if(millis()-startTime >= TIMEOUT_WAYPOINTS)
                    return -1;
            
            
            //if incoming bytes, read them and add them to the buffer
            if (Serial.available()){
                int incoming_char = Serial.read();
                buffer = buffer + char(incoming_char);
                counter++;
                if (incoming_char == '\n' || incoming_char == '\r'){
                
                if (buffer[0] == 'W'){
                    /*Finding the integers in the buffer, beginning at
                     start of LON, and then the start of LAT
                     
                     create substring starting at the characters after the space
                            |
                            v
                     " W LAT LON "
                         ^
                         |
                     Then go to index 2 of the string which will always be LAT and read chars, but this time stop reading at the next space.
                     
                     */
                    lon = buffer.substring(buffer.lastIndexOf(' '));
                    lat = buffer.substring(2,buffer.lastIndexOf(' '));
                    //convert to an int
                    lon = atoi(lon);
                    lat = atoi(lat);
                ...
                // If all waypoints have been recieved and we have validated 'E',
                //  return the number of bytes read
                if (buffer[0] == 'E') return counter;
            }
            }
            }
        }
